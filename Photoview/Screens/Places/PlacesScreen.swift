    ...
    var body: some View {
        ...
        .onChange(of: selectedAnnotation) { newAnnotation in
            if case let .image(marker, _) = newAnnotation {
                ...
                let thumbnail = MediaItem.Thumbnail(url: marker.properties.thumbnail.url, width: marker.properties.thumbnail.width, height: marker.properties.thumbnail.height)
                let mediaItem = MediaItem(id: GraphQLID(marker.properties.media_id), type: .photo, blurhash: nil, thumbnail: thumbnail, favorite: false)
                mediaEnv.media = [mediaItem]
            }
        }
    }
}
